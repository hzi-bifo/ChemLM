...
class Chemlm_model(nn.Module):
    def __init__(self, dt_n,drop_rate,an, att, n_layers, tt,seed_,emb_type, mo_path):   
        ...
        self.bert = RobertaModel.from_pretrained('{}'.format(mo_path),config=config)  
...
def tasks(prop,  save_path): 
	...
	stats(prop, df, 'chemlm')
	stats(prop, df_mol, 'molbert')
...
def calc_lip(props,save_path, model_path, model_path_, t_path): #calculates the ratio of property to embeddings distance for molecular pairs. This is for the ChemLM approach. The same method has been used in MolBERT on their code. 
	...
	model = Chemlm_model(dt_n='bbbp',drop_rate=0.1,an=80, att=12,n_layers= 8, tt='clf',seed_=3407,emb_type='last', mo_path = model_path)
